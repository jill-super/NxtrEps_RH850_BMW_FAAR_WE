...
* 07/11/17   4       SPP         Remove application data types                                                  EA4#12461
* 01/13/18   5       AR          MotVel Robustness Improvements                                                 EA4#19345
* 01/19/18   6       AJM         Change the reference counter limits                                            EA4#19630
**********************************************************************************************************************/
 ...
 *********************************************************************************************************************/
 ...
 VAR(uint8,  AUTOMATIC)  RollOvrOccured_Cnt_T_u08 = 0U;
 VAR(uint8,  AUTOMATIC)  HiPrsnt_Cnt_T_u08 = 0U;
 VAR(uint8,  AUTOMATIC)  LoPrsnt_Cnt_T_u08 = 0U;
 VAR(uint8,  AUTOMATIC)  MinIdx_Cnt_T_u08 = 0U;
 VAR(uint32,  AUTOMATIC)  RefTi_MicroSec_T_u32 = 65536U;
 ...
 /**************************************************************************************************/
	...
	{
		MotAgBuf_MotRev_T_u0p16[i] = (uint16)(MotAgTiBuf_MicroSec_T_u32[i] & 0x0000FFFFU);
		MotAgTiBuf_MicroSec_T_u32[i] = ((MotAgTiBuf_MicroSec_T_u32[i] & 0xFFFF0000U) >> 16U);
	}
	
	for(i = 0U; i < BUFSIZE_CNT_U08; i++)
	{
		if(MotAgTiBuf_MicroSec_T_u32[i] > 64800U)
		{
			HiPrsnt_Cnt_T_u08 = 1U;
		}
		if(MotAgTiBuf_MicroSec_T_u32[i] < 700U)
		{
			LoPrsnt_Cnt_T_u08 = 1U;
		}
	}
	 if((LoPrsnt_Cnt_T_u08 == 1) && (HiPrsnt_Cnt_T_u08 == 1))
	 {
		 RollOvrOccured_Cnt_T_u08 = 1U;
	 }
	for(i = 0U; i < BUFSIZE_CNT_U08 ; i++)
	{
		if(RollOvrOccured_Cnt_T_u08 != 0U)
		{
			if((MotAgTiBuf_MicroSec_T_u32[i] > 64800U) && (RefTi_MicroSec_T_u32 > MotAgTiBuf_MicroSec_T_u32[i]))
			{
				MinIdx_Cnt_T_u08 = i;
				RefTi_MicroSec_T_u32 = MotAgTiBuf_MicroSec_T_u32[i];
			}
		}
		else
		{
			if(RefTi_MicroSec_T_u32 > MotAgTiBuf_MicroSec_T_u32[i])
			{
				MinIdx_Cnt_T_u08 = i;
				RefTi_MicroSec_T_u32 = MotAgTiBuf_MicroSec_T_u32[i];
			}
	...
	MotAgBufIdx_Cnt_T_u08 = MinIdx_Cnt_T_u08;
