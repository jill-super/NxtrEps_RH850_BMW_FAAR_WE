...
* Module Description: Temporal Monitor Function - ES005C
* Project           : CBD
* Author            : Krishna Anne
***********************************************************************************************************************
* Version Control:
...
* --------  -------  --------  ---------------------------------------------------------------------------   -----------
* 03/22/17   1       KK        Initial version                                                                EA4#10206
* 03/27/17   2       KK        Updates as per FDD v1.1.0                                                      EA4#10206
* 08/08/17   3       SPP       Remove application data types                                                  EA4#13813
*-----------------------------------------------------------------------------------------------------------------------
**********************************************************************************************************************/
...
#include "Rte_TmplMonr.h" /* PRQA S 0857 */ /* MD_MSR_1.1_857 */
...
/* MISRA-C:2004 Rule 16.10 [NXTRDEV 16.10.1]: There is no appropriate action to take on the return value for Rte_Read, Rte_Write, Rte_Call_IoHwAb_SetGpio, Rte_Call_IoHwAb_GetGpio, all SPI driver functions, Rte_IrvWrite and SetNtcSts.  */ 
/* MISRA-C:2004 Rule 16.10 [NXTRDEV 16.10.2]: Server runnable is a void functon; Rte generation adds standard return type 
    but no error information is returned for Rte_Call_CtrlErrOut, Rte_Call_GetTiSpan1MicroSec32bit and Rte_Call_GetRefTmr1MicroSec32bit.  */ 



/********************************************* Embedded Local Constants **********************************************/
#define TMPLMONRAVRGCON_CNT_U08   		   (2U)
#define TMPLMONCNTR14SEL_CNT_U08   		   (14U)
#define TMPLMONCNTR17SEL_CNT_U08   		   (17U)
#define TMPLMONR50MICROSECTHD_CNT_U08      (50U)
#define TMPLMONRSPITRSMNTCTHD_CNT_U08 	   (250U)
#define TMPLMONRNTC40PRMBYTETHD_CNT_U08    (255U)
...
static FUNC(void, TmplMonr_CODE) TMFInitTestCase20(void); 
static FUNC(void, TmplMonr_CODE) TMFInitTestCase21(void); 
static FUNC(void, TmplMonr_CODE) TMFInitTestCase53(void); 
static FUNC(void, TmplMonr_CODE) SpiAsyncTx(Spi_ChannelType Channel_Cnt_T_u16, 
 ...
 *********************************************************************************************************************/
 ...
 /* Temporary variables */
 VAR(uint32, AUTOMATIC)   TmplMonrElpdTi_Cnt_T_u32;
 ...
 {	
 	...
 	(void)Rte_Call_IoHwAb_GetGpioPwrOutpEnaFb_Oper( &PwrOutpEnaFb_Cnt_T_logl ); 
 	...
 	else
 	{
 		...
 		{
 			if(SPI_SEQ_PENDING != Spi_GetSequenceResult(SpiConf_SpiSequence_TmplMonrSeq1))
			{
				/* Send spi read command to PwrSply IC */
				SpiAsyncTx(SpiConf_SpiChannel_TmplMonrCh1, TMPLMONRSTSREG2READVAL_CNT_U16, SpiConf_SpiSequence_TmplMonrSeq1);
				*Rte_Pim_TmplMonrSpiReadWrOrderFlg2() = TRUE;
			}
			else
			{
				/* Overfollow or rollover is intentional */
				*Rte_Pim_TrsmErrCntr() 	= (*Rte_Pim_TrsmErrCntr() + 1U); 
			}
 		}
 		else
 		{
 			if(SPI_SEQ_PENDING != Spi_GetSequenceResult(SpiConf_SpiSequence_TmplMonrSeq1))
			{
				...
				{
					/*** WD_STATE = Watchdog (7) ***/
					(void)Rte_Call_SetNtcSts_Oper( NTCNR_0X041, 1U, NTCSTS_FAILD, 0U );
				}
				*Rte_Pim_TmplMonrSpiReadWrOrderFlg2() = FALSE;
			}
			else
 ...
 else if( StrtUpSt_Cnt_T_u08 == ELECGLBPRM_STRTUPSTTMPLMONININTESTSTRT_CNT_U08 )
 {
	...
	/* SpiErrDiag */
	if(*Rte_Pim_TrsmErrNtcThd() == TMPLMONRSPITRSMNTCTHD_CNT_U08)
	{
		*Rte_Pim_TmplMonrNtc40PrmByte() 	= TMPLMONRNTC40PRMBYTETHD_CNT_U08;
 ...
 (void)Rte_Call_GetRefTmr1MicroSec32bit_Oper(&TmplMonrElpdTi_Cnt_T_u32);
 (void)Rte_IrvWrite_TmplMonrPer1_ElpdTiMicroSec(TmplMonrElpdTi_Cnt_T_u32);
 ...
 *********************************************************************************************************************/
 ...
 (void)Rte_Call_GetTiSpan1MicroSec32bit_Oper(Rte_IrvRead_TmplMonrPer2_ElpdTiMicroSec(), &TiSpan_Cnt_T_u32);
 
 *Rte_Pim_AvrgElpdTiMicroSec() = ((TiSpan_Cnt_T_u32 + *Rte_Pim_AvrgElpdTiMicroSec())/ TMPLMONRAVRGCON_CNT_U08);
 ...
 *********************************************************************************************************************/
 ...
 if(SysSt_Cnt_T_enum == SYSST_OFF)
 {
	...
	while(TiSpan_Cnt_T_u32 < TMPLMONR50MICROSECTHD_CNT_U08);
...
static FUNC(void, TmplMonr_CODE) TMFInitTest(void)
{
	switch(*Rte_Pim_TmplMonrIninCntr())
	{
		...
		case 12U:  
			...
			(void)Rte_Call_IoHwAb_SetGpioTmplMonrWdg_Oper(STD_LOW);
			/* Set nErr pin to Low */
			(void)Rte_Call_CtrlErrOut_Oper((STD_LOW), TRIGREG_MSTANDCHKR);
			*Rte_Pim_TmplMonrIninCntr() = *Rte_Pim_TmplMonrIninCntr() + 1U;
		break;
		...
		case 14U:		
			...
			(void)Rte_Call_CtrlErrOut_Oper((STD_HIGH), TRIGREG_MSTANDCHKR);
			/* Reset GPIO TmplMonrWdg pin to Low */
			(void)Rte_Call_IoHwAb_SetGpioTmplMonrWdg_Oper(STD_LOW);
			*Rte_Pim_TmplMonrIninCntr() = *Rte_Pim_TmplMonrIninCntr() + 1U;
		break;
...
{
	if( (SPI_SEQ_PENDING != Spi_GetSequenceResult(SpiConf_SpiSequence_TmplMonrSeq1)) &&
	    (SPI_SEQ_PENDING != Spi_GetSequenceResult(SpiConf_SpiSequence_TmplMonrSeq2)) &&
	    (SPI_SEQ_PENDING != Spi_GetSequenceResult(SpiConf_SpiSequence_TmplMonrSeq3)) )
		{
			/*** START: Send WatchDog restart command through spi ***/
			/* Transmit "Watchdog Mode Key: Word1" command */
			SpiAsyncTx(SpiConf_SpiChannel_TmplMonrCh1, TMPLMONRWDGMODKEYWORD1_CNT_U16, SpiConf_SpiSequence_TmplMonrSeq1);
					
			/* Transmit "Watchdog Mode Key: Word2" command */
			SpiAsyncTx(SpiConf_SpiChannel_TmplMonrCh2, TMPLMONRWDGMODKEYWORD2_CNT_U16, SpiConf_SpiSequence_TmplMonrSeq2);
					
			/* Transmit "Watchdog Mode Key: Word3" command */
			SpiAsyncTx(SpiConf_SpiChannel_TmplMonrCh3, TMPLMONRWDGMODKEYWORD3_CNT_U16, SpiConf_SpiSequence_TmplMonrSeq3);
		...
		{
			*Rte_Pim_TrsmErrNtcThd() 	= Min_u08((*Rte_Pim_TrsmErrNtcThd() + 1U), TMPLMONRSPITRSMNTCTHD_CNT_U08);
		}
		(void)Rte_Call_IoHwAb_SetGpioTmplMonrWdg_Oper(STD_LOW);
...
static FUNC(void, TmplMonr_CODE) TMFInitTestCase10To11Case15To17(void)
{
	...
	if( *Rte_Pim_TmplMonrIninCntr() < TMPLMONCNTR17SEL_CNT_U08)
	{
		/* Get the pin state of  POE */
		(void)Rte_Call_IoHwAb_GetGpioPwrOutpEnaFb_Oper( &PwrOutpEnaFb_Cnt_T_logl );
		...
		{	/* Fault Condition - WD Signal */
			...
			if( *Rte_Pim_TmplMonrIninCntr() > TMPLMONCNTR14SEL_CNT_U08)
			{
...
static FUNC(void, TmplMonr_CODE) TMFInitTestCase19(void)
{
	...
	if(SPI_SEQ_PENDING != Spi_GetSequenceResult(SpiConf_SpiSequence_TmplMonrSeq1))
	{
		...
		{	
			...
			if(SPI_SEQ_PENDING != Spi_GetSequenceResult(SpiConf_SpiSequence_TmplMonrSeq2))
			{
				SpiAsyncTx(SpiConf_SpiChannel_TmplMonrCh2, TMPLMONRCFGREG1WRVAL_CNT_U16, SpiConf_SpiSequence_TmplMonrSeq2);
				*Rte_Pim_TmplMonrIninCntr() = *Rte_Pim_TmplMonrIninCntr() + 1U;
			}
			else
			{
...
/**********************************************************************************************************************
* Name:        TMFInitTestCase20
* Description: Temporal Monitor Initialization Test Case 20
* Inputs:      None
* Outputs:     None
* Usage Notes: None
**********************************************************************************************************************/
...
{
	/* Send spi read command to PwrSply IC */
	if(SPI_SEQ_PENDING != Spi_GetSequenceResult(SpiConf_SpiSequence_TmplMonrSeq3))
	{

		SpiAsyncTx(SpiConf_SpiChannel_TmplMonrCh3, TMPLMONRCFGREG1READVAL_CNT_U16, SpiConf_SpiSequence_TmplMonrSeq3);
		*Rte_Pim_TmplMonrIninCntr() = *Rte_Pim_TmplMonrIninCntr() + 1U;
...
/**********************************************************************************************************************
* Name:        TMFInitTestCase21
* Description: Temporal Monitor Initialization Test Case 21
* Inputs:      None
* Outputs:     None
* Usage Notes: None
**********************************************************************************************************************/
static FUNC(void, TmplMonr_CODE) TMFInitTestCase21(void)
{
	...
	if(SPI_SEQ_PENDING != Spi_GetSequenceResult(SpiConf_SpiSequence_TmplMonrSeq3))
	{
		...
		{	/* Edge/Valid Cnts are NOT Correct */
			*Rte_Pim_TmplMonrNtc40PrmByte() = *Rte_Pim_TmplMonrNtc40PrmByte() | 32U;
		}
		*Rte_Pim_TmplMonrIninCntr() = *Rte_Pim_TmplMonrIninCntr() + 1U;
	}
	else
...
static FUNC(void, TmplMonr_CODE) TMFInitTestCase53(void)
{
	...
	{	/* Normal Condition */
		...
		/* Write module o/p */
		(void)Rte_Write_TmplMonrIninTestCmpl_Logl(TRUE);
		*Rte_Pim_TmplMonrIninTestCmplFlg() = TRUE;
		*Rte_Pim_TmplMonrSpiReadWrOrderFlg1() = FALSE;
		*Rte_Pim_TmplMonrIninCntr() = *Rte_Pim_TmplMonrIninCntr() + 1U;
	}
