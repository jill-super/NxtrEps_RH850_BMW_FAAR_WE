/**********************************************************************************************************************
 *  COPYRIGHT
 *  -------------------------------------------------------------------------------------------------------------------
 *
 *                This software is copyright protected and proprietary to Vector Informatik GmbH.
 *                Vector Informatik GmbH grants to you only those rights as set out in the license conditions.
 *                All other rights remain with Vector Informatik GmbH.
 *  -------------------------------------------------------------------------------------------------------------------
 *  FILE DESCRIPTION
 *  -------------------------------------------------------------------------------------------------------------------
 *          File:  TSC_BmwMsgSlot49Bas1Repn2BusFrChA.c
 *        Config:  EPS.dpa
 *   ECU-Project:  EPS
 *
 *     Generator:  MICROSAR RTE Generator Version 4.16.0
 *                 RTE Core Version 1.16.0
 *       License:  Unlimited license CBD1700369 for Nexteer Automotive Corporation
 *
 *   Description:  Implementation of wrapper software component for Bte-based Rte test cases
 *********************************************************************************************************************/
#include "Rte_BmwMsgSlot49Bas1Repn2BusFrChA.h"
#include "TSC_BmwMsgSlot49Bas1Repn2BusFrChA.h"















     /* Client Server Interfaces: */


     /* Service calls */


     /* Mode Interfaces */




     /* Inter-Runnable variables */






Std_ReturnType TSC_BmwMsgSlot49Bas1Repn2BusFrChA_Rte_Read_BattRtnCurrAmpr_Val(float32 *data)
{
  return Rte_Read_BattRtnCurrAmpr_Val(data);
}

Std_ReturnType TSC_BmwMsgSlot49Bas1Repn2BusFrChA_Rte_Read_BattRtnCurrAmprQlfr_Val(BattRtnCurrAmprQlfr1 *data)
{
  return Rte_Read_BattRtnCurrAmprQlfr_Val(data);
}

Std_ReturnType TSC_BmwMsgSlot49Bas1Repn2BusFrChA_Rte_Read_EpsCurrLimSts_Val(EpsCurrLimSts1 *data)
{
  return Rte_Read_EpsCurrLimSts_Val(data);
}




Std_ReturnType TSC_BmwMsgSlot49Bas1Repn2BusFrChA_Rte_Write_sigGroup_DT_EST_sigGroup_DT_EST(const sigGroup_DT_EST1 *data)
{
  return Rte_Write_sigGroup_DT_EST_sigGroup_DT_EST(data);
}





     /* Client Server Interfaces: */


     /* Service calls */


     /* Mode Interfaces */




     /* Inter-Runnable variables */







     /* BmwMsgSlot49Bas1Repn2BusFrChA */
      /* BmwMsgSlot49Bas1Repn2BusFrChA */



