...
{
  return Rte_Mode_DataMode_currentDataMode();
}
...
{
  return Rte_Read_ProgId_progId(data);
}
...
{
  return Rte_Mode_BusCom_Mode();
}
uint8 TSC_Coding_Rte_Mode_ConditionMode_currentConditionMode(void)
{
  return Rte_Mode_ConditionMode_currentConditionMode();
}
...
{
  return Rte_Switch_DataMode_currentDataMode( mode);
}
...
{
  return Rte_Call_CheckCurrentLimit_CheckPlausibility(Value);
}
...
{
  return Rte_Call_NvMService_ReadBlock(DstPtr);
}
...
{
  return Rte_IRead_VinReceiveHandler_Vin_Vin();
}
