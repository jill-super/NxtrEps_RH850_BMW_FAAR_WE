...
FEE_30_SMALLSECTOR_LOCAL FUNC(void, FEE_30_SMALLSECTOR_PRIVATE_CODE) Fee_30_SmallSector_If_ProcessCheckErasePattern1(void)
{
  ...
  if ((Fee_30_SmallSector_Fls_GetJobResult() == MEMIF_JOB_OK) && (Fee_30_SmallSector_Dh_IsErasePatternBlank() == TRUE) ) /* PRQA S 3415 */ /* MD_FEE_30_SMALLSECTOR_12.4 */
  {
    ...
    /* #50 Finish InstanceFinder job if BlankCheck either returned with MEMIF_BLOCK_INCONSISTENT (ErasePattern1 is not blank/erased) or if the job failed  */
    switch (Fee_30_SmallSector_Fls_GetJobResult())
    {
    case MEMIF_JOB_FAILED:
      Fee_30_SmallSector_If_FinishJob(INSTANCE_FINDER_FAILED);
      break;
    default: /* #60 Either MEMIF_JOB_OK, if BlankCheck API is disabled and ErasePattern is not erased, or MEMIF_BLOCK_INCONSISTENT, if BlankCheck detects non blank pattern */
...
FEE_30_SMALLSECTOR_LOCAL FUNC(void, FEE_30_SMALLSECTOR_PRIVATE_CODE) Fee_30_SmallSector_If_ProcessReadErasePattern2(void)
{
  if ((Fee_30_SmallSector_Fls_GetJobResult() == MEMIF_JOB_OK) && (Fee_30_SmallSector_Dh_IsErasePatternValid() == TRUE)) /* PRQA S 3415 */ /* MD_FEE_30_SMALLSECTOR_12.4 */
  {
  ...
  else
  {
    ...
    case MEMIF_JOB_FAILED:
      Fee_30_SmallSector_If_FinishJob(INSTANCE_FINDER_FAILED);
      break;
    default: /* #30 Result is mapped to MEMIF_BLOCK_INVALID so that InstanceFinder result will be interpreted correctly by DatasetEraser sub-component */
      Fee_30_SmallSector_If_FinishJob(INSTANCE_FINDER_EP2_INCORRECT);
      break;
    }
  }
