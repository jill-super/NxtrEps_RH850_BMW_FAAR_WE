...
* Module Description: Implementation of HwTqCorrln FDD ES229B
* Project           : CBD 
...
* -------   -------  --------  ----------------------------------------------------------------------------------------
* 05/24/2016  	1        BG       		Initial Version                                                       EA4#5382    
**********************************************************************************************************************/
...
#define    HWTQIDPTSIGZERO_CNT_U08                 0U
#define    HWTQIDPTSIGTWO_CNT_U08                  2U           
 ...
 *********************************************************************************************************************/
     ...
     /** ChA Common Mode Compensation LPF Init **/ 
     /* ChA CMC LPF Init - Fc  */  
      /* The CMC LPF is initialized for its Fc and State variable is value obtained from NVM*/
     FilLpInit(*Rte_Pim_HwTqChACmnModCmpLpFilSt(), Rte_Prm_HwTqCorrlnChACmnModCmpLpFilFrq_Val(), ARCHGLBPRM_100MILLISEC_SEC_F32, Rte_Pim_HwTqChACmnModCmpLpFil()); /*REQ: ES229B #205 I*/
    
     /* ChA Steady State LPF Init */ 
     FilLpInit(0.0F, Rte_Prm_HwTqCorrlnChAStabStLpFilFrq_Val(), ARCHGLBPRM_100MILLISEC_SEC_F32, Rte_Pim_HwTqChAStabStLpFil()); /*REQ: ES229B #227 I*/
 ...
 *********************************************************************************************************************/
    ...
    VAR(boolean, AUTOMATIC)    HwTqChAAvl_Cnt_T_lgc;  	        
    ...
    SigAAvail_Cnt_T_lgc = CorrlnSigAvlChk(HwTqARollgCntr_Cnt_T_u08, HwTqAQlfr_Cnt_T_enum, MAXSTALLTQA_CNT_U08, /* HwTq Signal A */ 
                                          Rte_Pim_HwTqARollgCntrPrev(), Rte_Pim_HwTqAStallCntrPrev());    	/*REQ: ES229B #211 I*/										  

	

    SigBAvail_Cnt_T_lgc = CorrlnSigAvlChk(HwTqBRollgCntr_Cnt_T_u08, HwTqBQlfr_Cnt_T_enum, MAXSTALLTQB_CNT_U08, /* HwTq Signal B */ 
                                          Rte_Pim_HwTqBRollgCntrPrev(), Rte_Pim_HwTqBStallCntrPrev());		/*REQ: ES229B #212 I*/

	/*REQ: ES229B #213 I*/										  
	/*REQ: ES229B #216 I*/
										  
 /* Signals Available */ 
    if ((SigAAvail_Cnt_T_lgc == TRUE) && (SigBAvail_Cnt_T_lgc == TRUE))
    { 
        TqSum_HwNwtMtr_T_f32 = Abslt_f32_f32((HwTqA_HwNwtMtr_T_f32 - HwTqB_HwNwtMtr_T_f32) / (float32)HWTQSCA_CNT_U08);
        
        /* Imdt Corrln Chk Failed */
        if(TqSum_HwNwtMtr_T_f32 >= Rte_Prm_HwTqCorrlnChAImdtCorrlnChkFailThd_Val())
        {
            *Rte_Pim_HwTqChAImdtCorrlnChk() = FALSE;
        }
        /* Imdt Corrln Chk Passed */		 
        else if(TqSum_HwNwtMtr_T_f32 <= Rte_Prm_HwTqCorrlnChAImdtCorrlnChkPassThd_Val())
        {
            *Rte_Pim_HwTqChAImdtCorrlnChk() = TRUE;
        }
        else
        {
            /* Hold last value */
    ...
    {
        *Rte_Pim_HwTqChAImdtCorrlnChk() = FALSE;
        HwTqChAAvl_Cnt_T_lgc = FALSE;
    } 
	/*ENDREQ: ES229B #216 */	

    (void)Rte_Call_GetNtcQlfrSts_Oper(NTCNR_0X070,  &NtcQlfrStatus_T_enum);   
    
	
	/*REQ: ES229B #215 I*/
    /* Determine Correlation Status Bits */
    /* Set Corrln Bits */
    if((*Rte_Pim_HwTqChAImdtCorrlnChk() == TRUE) && (NtcQlfrStatus_T_enum != SIGQLFR_FAILD))
    {
        CorrlnSigAPass_Cnt_T_lgc = TRUE;
        CorrlnSigBPass_Cnt_T_lgc = TRUE;
    }
    /* Clear Corrln Bits */
    else
    {
        CorrlnSigAPass_Cnt_T_lgc = FALSE;
        CorrlnSigBPass_Cnt_T_lgc = FALSE;
    }  			
    ...
    {
        HwTqCorrlnSts_Cnt_T_u08 = HwTqCorrlnSts_Cnt_T_u08 | HWTQCORRLNSTSSIGB_CNT_U08;
    }   

    
    HwTqCorrlnSts_Cnt_T_u08 = Lim_u08(HwTqCorrlnSts_Cnt_T_u08, HWTQCORRLNSTSMINLMT_CNT_U08, HWTQCORRLNSTSMAXLMT_CNT_U08);   
    
    /*Write Outputs */
    (void) Rte_Write_HwTqCorrlnSts_Val(HwTqCorrlnSts_Cnt_T_u08);    
	(void) Rte_IrvWrite_HwTqCorrlnPer1_HwTqChAAvl(HwTqChAAvl_Cnt_T_lgc);
 ...
 *********************************************************************************************************************/
    ...
    /* Ch A Variable Fc Transient Fault LPF */ 
    ...
    /* Independent Signals Determination */ /*REQ: ES229B #220 I*/ /*REQ: ES229B #219 I*/ /*REQ: ES229B #218 I*/
    (void)Rte_Call_GetNtcQlfrSts_Oper(NTCNR_0X070,  &ChANtcActv_T_enum);  


	if ((ChAAvl_Cnt_T_lgc == TRUE) && (ChANtcActv_T_enum != SIGQLFR_FAILD))
	{
		HwTqIdptSig_Cnt_T_u08 = HWTQIDPTSIGTWO_CNT_U08;
	}
	else
	{
		HwTqIdptSig_Cnt_T_u08 = HWTQIDPTSIGZERO_CNT_U08;
	}
	  
    
    HwTqIdptSig_Cnt_T_u08 = Lim_u08(HwTqIdptSig_Cnt_T_u08, HWTQIDPDTSIGMINLMT_CNT_U08, HWTQIDPDTSIGMAXLMT_CNT_U08); 
    
    /* Fault Injection Point */
    #if(STD_ON == FLTINJENA)
    Rte_Call_FltInj_u08_Oper(&HwTqIdptSig_Cnt_T_u08,FLTINJ_IDPTSIG_HWTQCORRLN); /* REQ: ES229B #206 I */
    #endif
 ...
 *********************************************************************************************************************/
    ...
    /* Read Inputs */
    (void)Rte_Read_HwTqChA_Val(&HwTqChA_HwNwtMtr_T_f32);              
	ChATqSumLim_HwNwtMtr_T_f32 = Rte_IrvRead_HwTqCorrlnPer3_HwTqChATqSumLim();
     
	
    /*** ChA Long Term Offset Compensation ***/
    LpFilOutpA_HwNwtMtr_T_f32 = FilLpUpdOutp_f32(ChATqSumLim_HwNwtMtr_T_f32, Rte_Pim_HwTqChAStabStLpFil());  
    ...
    HwTqChACmnModCmpLpFilSt_Uls_T_f32 = FilLpUpdOutp_f32(ChATqSumLim_HwNwtMtr_T_f32, Rte_Pim_HwTqChACmnModCmpLpFil());
	
	/*REQ: ES229B #226 I*/ /*REQ: ES229B #225 I*/ /*REQ: ES229B #224 I*/
    if((Abslt_f32_f32(HwTqChA_HwNwtMtr_T_f32)) < Rte_Prm_HwTqCorrlnChACmnModCmpLpFilActvnThd_Val())
    {       
        /* Ch A CMC LPF Update Outp */
        *Rte_Pim_HwTqChACmnModCmpLpFilSt() = HwTqChACmnModCmpLpFilSt_Uls_T_f32; 
    }
    else
    {
        /* Ch A CMC LPF Hold Last Val */
        *Rte_Pim_HwTqChACmnModCmpLpFilSt() = Rte_Pim_HwTqChACmnModCmpLpFil()->FilSt; 
    }
    
	(void)Rte_Call_HwTqChACmnModCmpLpFilSt_SetRamBlockStatus(TRUE); 
	/*ENDREQ: ES229B #227 */ 	/*ENDREQ: ES229B #229 */	
